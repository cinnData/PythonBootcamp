...
3. As a continuation of the preceding exercise, create a list containing the ID's of the big hosts, with more than 100 listings. Then, extract a data subset containing only the listings managed by the big hosts. The method `.isin()` can help you to write the appropriate filter condition. Do you find the big hosts focused on certain neighbourhoods? Do they have higher prices?
